...
def index(request):
    return render(request, "dashboard.html")  # 确保返回dashboard.html
...
def get_recommend(request):
    ...
    # 调用推荐算法获取推荐职位
    recommend_jobs = job_recommend.recommend_by_item_id(user_id)
...
def get_recommend_partial(request):
    """首页部分推荐接口：返回JSON格式的推荐数据"""
    user_id = request.session.get('user_id')
    if not user_id:
...
def get_chart_data(request):
    ...
    edu_data = []
    for edu in edu_list:
        edu_count = len(models.JobData.objects.filter(education__icontains=edu))
    ...
    list_50 = []
    list_51 = []
    job_data = models.JobData.objects.all().values()
    for job in list(job_data):
        try:
            salary_1 = float(re.findall(r'-(\d+)k', job['salary'])[0])
            if salary_1 <= 5:
                list_5.append(salary_1)
            elif 10 >= salary_1 > 5:
                list_10.append(salary_1)
    ...
    salary_data = [
        ...
        {'name': '15K-20K', 'value': len(list_20)},
        {'name': '20K-30K', 'value': len(list_30)},
        {'name': '30K-50K', 'value': len(list_50)},
        {'name': '50K以上', 'value': len(list_51)}
    ]

    # 工作经验分布数据
    exp_list = ['应届', '1-3年', '3-5年', '5-10年', '10年以上']
    exp_data = []
    for exp in exp_list:
        exp_count = len(models.JobData.objects.filter(experience__icontains=exp))
        exp_data.append({'name': exp, "value": exp_count})

    # 热门城市数据
    city_list = ['北京', '上海', '广州', '深圳', '杭州', '成都', '南京', '武汉', '西安']
    city_data = []
    for city in city_list:
        city_count = len(models.JobData.objects.filter(place__icontains=city))
        city_data.append({'name': city, "value": city_count})
...
from .models import JobData  # 导入职位数据模型
import re

def expect(request):
    # 初始化上下文变量
    context = {
        'expect_data': {},  # 存储用户提交的期望条件
        'city_list': ['北京', '上海', '广州', '深圳', '杭州', '成都'],  # 可选城市列表
        'matched_jobs': []  # 匹配的职位结果
    }

    # 如果是POST请求（用户提交了期望条件）
    if request.method == 'POST':
        # 获取表单提交的参数
        job_keyword = request.POST.get('job_keyword', '').strip()
        city = request.POST.get('city', '').strip()
        salary_expect = request.POST.get('salary_expect', '').strip()
        ...
        # 从数据库查询符合条件的职位（核心逻辑）
        matched_jobs = JobData.objects.all()  # 先获取所有职位

        # 1. 按关键词筛选（职位名称包含关键词）
        if job_keyword:
            matched_jobs = matched_jobs.filter(job_name__icontains=job_keyword)

        # 2. 按城市筛选
        if city:
            matched_jobs = matched_jobs.filter(city=city)

        # 3. 按学历筛选
        if education:
            matched_jobs = matched_jobs.filter(education__icontains=education)

        # 4. 按期望薪资筛选（需解析薪资范围，这里简化处理）
        if salary_expect == '5k以下':
            matched_jobs = matched_jobs.filter(salary__regex=r'^[0-4]\d*k')
        elif salary_expect == '5k-10k':
            matched_jobs = matched_jobs.filter(salary__regex=r'^[5-9]\d*k|10k')
        elif salary_expect == '10k-20k':
            matched_jobs = matched_jobs.filter(salary__regex=r'^1[1-9]k|20k')
        # 可继续扩展其他薪资区间的筛选逻辑

        # 将匹配结果存入上下文
        context['matched_jobs'] = matched_jobs

    # 渲染模板（传递上下文变量给expect.html）
    return render(request, 'expect.html', context)
...
# 指定 msedgedriver 的路径
driver_path = os.path.join(current_dir, 'msedgedriver.exe')  # 确保 msedgedriver 文件名是正确的
...
def get_city():
    ...
    edge_options.add_argument('--headless')  # 运行在无头模式
    # 使用 Service 指定 msedgedriver 路径
...
def get_mysql():
    ...
    CREATE TABLE IF NOT EXISTS job_data (
        ...
        label VARCHAR(100) COMMENT '公司标签',
        scale VARCHAR(100) COMMENT '公司规模',
        href VARCHAR(255) NOT NULL UNIQUE COMMENT '职位详情链接',
        key_word VARCHAR(100) COMMENT '搜索关键词',
